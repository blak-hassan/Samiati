...
import { reportSchema, ReportFormData } from '@/lib/schemas';
...
export const ReportModal: React.FC<Props> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleFormSubmit = (data: ReportFormData) => {
    const reasonLabel = REASONS.find((r) => r.id === data.reason)?.label || 'Other';
    onSubmit(reasonLabel, data.details);
  };
      ...
      <div className="bg-white dark:bg-[#32241a] w-full max-w-md rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-stone-500 dark:text-text-muted">
                  Report low quality or issues
                </p>
              </div>
            </div>
            <button
          ...
          <div className="space-y-3 mb-6">
            {REASONS.map((reason) => (
              <button
                key={reason.id}
                type="button"
                onClick={() => setValue('reason', reason.id, { shouldValidate: true })}
              ...
              >
                <div className="flex items-center justify-between mb-0.5">
                  <span
                    className={`text-sm font-black ${
                      selectedReason === reason.id ? 'text-rasta-red' : 'text-stone-900 dark:text-white'
                    }`}
                  >
                    {reason.label}
                  </span>
                ...
                <p className="text-[10px] font-medium text-stone-500 dark:text-text-muted">{reason.desc}</p>
              </button>
            ))}
          </div>
          {errors.reason && <p className="text-destructive text-xs font-medium mb-4">{errors.reason.message}</p>}
          ...
          <div className="flex gap-3">
            ...
            <button
              disabled={!selectedReason || (selectedReason === 'other' && !watch('details')?.trim())}
