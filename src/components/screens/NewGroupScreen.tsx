...
const NewGroupScreen: React.FC<Props> = ({ navigate, goBack, onCreateGroup }) => {
    ...
    const searchKeys = React.useMemo(() => ['name'], []);
    const filteredContacts = useFuzzySearch(MOCK_CONTACTS, searchQuery, searchKeys);
        ...
        <div className="flex flex-col h-full bg-white dark:bg-background-dark transition-colors duration-300">
            ...
            {selectedContacts.size > 0 && (
                <div className="fixed bottom-6 right-6 z-20">
                    <button
                        onClick={handleCreate}
                        className={`w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all duration-200 animate-in zoom-in ${isValid
