...
import { composePostSchema } from '@/lib/schemas';
import { z } from 'zod';
import { useUploadFile } from '@/hooks/useUploadFile';

type ComposeFormData = z.infer<typeof composePostSchema>;
...
const ComposePostScreen: React.FC<Props> = ({ navigate, goBack, onPost, user }) => {
  ...
  const pollDuration = watch('pollDuration');
  ...
  const handlePublish = async () => {
    if (!text.trim() && !imageFile && !showPoll) return;
    ...
    const postData: PostPayload = {
      visibility,
      cw: showCw && watch('cw').trim() ? watch('cw') : undefined,
    };

    if (showPoll) {
      const validOptions = watch('pollOptions')
        .map((o) => o.value.trim())
        .filter((o) => o.length > 0);
    ...
    onPost(text, imageStorageId, postData);
    goBack();
  };
  ...
  const getVisibilityIcon = () => {
    switch (visibility) {
      ...
      case 'Unlisted':
        return <LockKeyholeOpen className="w-4 h-4" />;
      case 'Followers':
    ...
    <div className="flex flex-col h-screen bg-background transition-colors duration-300">
      <header className="flex items-center justify-between px-4 py-2 border-b border-border bg-background/95 backdrop-blur-md sticky top-0 z-30">
        ...
        >
          ...
          disabled={(!text.trim() && !imageFile && !showPoll) || isUploading}
              ...
              <Card className="bg-muted/30 p-4 rounded-2xl border-primary/10 animate-in fade-in slide-in-from-top-2 duration-300 shadow-sm">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-xs font-black uppercase tracking-[0.15em] text-foreground/80">
                      Active Poll
                    </h4>
                  </div>
                  <Button
                  ...
                  {fields.map((field, idx) => (
                    <div key={field.id} className="flex gap-3 items-center">
                      <div className="flex-1 relative">
                        <Input
                          {...register(`pollOptions.${idx}.value`)}
                          ref={idx === 0 ? firstPollInputRef : undefined}
                ...
                <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
                  ...
                  <select
                    value={pollDuration}
                    onChange={(e) => setValue('pollDuration', e.target.value)}
          ...
          <div className="flex items-center gap-1">
            ...
            <Popover open={showVisibilityMenu} onOpenChange={setShowVisibilityMenu}>
              ...
              <PopoverContent
                side="top"
                align="start"
                className="w-[180px] p-1.5 rounded-2xl shadow-2xl border-border bg-background"
              >
                {(['Public', 'Unlisted', 'Followers', 'Direct'] as const).map((opt) => (
                  ...
                  >
                    <div
                      className={cn("w-4 h-4", visibility === opt ? "text-primary" : "text-muted-foreground")}
                    >
                      {opt === 'Public' ? (
