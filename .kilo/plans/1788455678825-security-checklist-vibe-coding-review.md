...
Scope: Review the supplied 27-item checklist, score its coverage, identify gaps, and deliver an expanded, categorized checklist tuned for fast-moving modern web apps where speed is prioritized over security.

---

## 1. Evaluation of the Original 27 Items

### Significance (H/M/L = impact when present in a vibe-coded app)

| # | Item | Sig | Rationale |
|---|------|-----|-----------|
| 1 | Exposed DB credentials | **H** | Direct breach vector; one leak = full DB exfiltration. |
| 2 | Public .env files | **H** | Common in early deploys (Vercel/Netlify/static hosts). Leaks everything in one file. |
| 3 | Weak authentication | **H** | Allows takeover via credential stuffing / brute force. |
| 4 | Hardcoded secrets | **H** | Persists in repo + image layers; very common in prototypes. |
...
| 7 | Open DB permissions | **H** | Public DB (esp. hosted Postgres/Redis/Mongo) = full compromise. |
| 8 | Cloud service misconfigurations | **H** | Public S3/GCS buckets, open security groups, permissive IAM. |
| 9 | Exposed production debug tools | **H** | `/debug`, `/_debug`, `/graphql` introspection, `/actuator`, Sentry replays. |
...
| 13 | Secrets in client-side JS | **H** | Bundle-leaked API keys, Stripe-style publishable vs. secret mistakes. |
...
| 16 | NoSQL injection | **H** | `$ne`, `$gt`, `$where` in Mongo; Knex-style query builders mis-used. |
| 17 | Insecure file uploads | **H** | RCE via polyglot files, path traversal, content-type bypass. |
| 18 | Weak session management | **H** | No rotation, long-lived sessions, missing Secure/HttpOnly/SameSite. |
| 19 | Broken password reset flows | **H** | Account takeover via token reuse, weak tokens, user-enumeration. |
| 20 | Path traversal | **M** | Common in file-serving endpoints, archive extraction, downloads. |
| 21 | Missing rate limits | **M** | Enables brute force, scraping, DoS. |
| 22 | Frontend-only payment / subscription logic | **H** | Anyone can bypass; trivially exploited. |
...
| 25 | CSRF | **M** | Lower in pure-JSON API + SameSite=Lite cookies, but state-changing endpoints still need protection. |
| 26 | Exposed source maps | **M** | Reveals original source paths, comments, sometimes secrets. |
| 27 | Unverified payment webhooks | **H** | Spoofed "payment succeeded" = free goods/services. |

**Verdict:** The original list is a strong baseline of classic web-app issues. It is biased toward:
- **Classic injection / authz** (good)
- **Secrets & infrastructure exposure** (good)
- **Payment business logic** (good — uniquely relevant to vibe-coded SaaS)

It is **light on** several categories critical for modern stacks: supply chain, cryptography, API/GraphQL, headers/CORS, AI/LLM-specific risks, observability, DoS, race conditions, and third-party integration hygiene. See Section 2.

---

## 2. Gaps in the Original List

Missing categories, in approximate priority order:

1. **Supply chain & build pipeline** — vulnerable deps, dependency confusion, malicious npm packages, unsafe CI, unsigned auto-updates.
2. **Cryptography & token design** — weak hashing (MD5/SHA1/unsalted SHA256), JWT misuse (alg:none, no exp/aud, weak secret), Math.random() for tokens, missing TLS hardening (HSTS, TLS 1.2+), weak/legacy ciphers.
4. **API / GraphQL specifics** — over-fetching, mass assignment / parameter pollution, missing depth/complexity limits, introspection in prod, BOLA at API layer (often missed as "IDOR").
5. **Security headers & browser hardening** — CSP, X-Frame-Options/frame-ancestors, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, COOP/COEP.
6. **CORS misconfiguration** — wildcard origins with credentials, dynamic origin reflection.
7. **Authentication internals** — OAuth/OIDC misuse (missing state/PKCE, open redirect in `redirect_uri`), session fixation, account enumeration, MFA bypass, long-lived tokens, JWT-in-localStorage.
8. **Authorization internals** — vertical/horizontal privilege escalation, missing function-level checks, tenant isolation in multi-tenant SaaS.
9. **Race conditions & business-logic concurrency** — double-spend, double-redeem, TOCTOU.
10. **Denial of service** — ReDoS, large payloads, expensive queries, slowloris, resource exhaustion, payment-flood.
11. **Observability & incident response** — insufficient security logging, no alerting on suspicious events, log injection, missing audit trails, no centralized SIEM.
12. **Data protection & privacy** — PII over-collection, no masking in non-prod, GDPR/CCPA, PII in logs, exposed backups, DB snapshots in public buckets.
13. **Third-party / integration hygiene** — overly permissive OAuth scopes, embedded iframes, third-party SDKs that phone home, open redirect chains, ad-network malvertising.
14. **Webhooks & async jobs** — replay attacks, missing idempotency keys, async job injection.
15. **File/storage** — symlink attacks, ZIP slip, XXE, prototype pollution, serving user uploads with permissive content-type (SVG/HTML).
16. **Deployment hygiene** — staging publicly reachable, test endpoints in prod, dev tooling shipped, secrets in Docker image layers.
17. **AI / LLM specific** — prompt injection, indirect prompt injection via fetched content/documents, training data leakage, model extraction, insecure output handling (LLM HTML rendered raw), expensive-prompt DoS, sensitive data sent to model providers.
18. **Subdomain takeover / DNS rebinding** — dangling CNAMEs to dead SaaS tenants; DNS rebinding against local/internal services.
19. **Clickjacking, postMessage, WebSocket origin validation** — separate from XSS.
20. **Compliance-specific** — PCI tokenization skipped, COPPA for minors' data, missing privacy/ToS links.

---

## 3. Expanded, Categorized Checklist

Each item has: **Severity** (H/M/L), **Why vibe-coders miss it**, **Quick check / control**.
...
- **A5. Secrets in Git history** (H) — `git log -p | grep`; rotate + BFG/rebase + force-push awareness.
...
### B. Authentication
- **B1. Weak auth / no password policy** (H) — HIBP check, length-based rules, no arbitrary complexity churn.
...
- **B11. JWT in localStorage** (M) — prefer httpOnly Secure SameSite cookies for session identity.
- **B12. Weak password reset flow** (H) — single-use, expiring (≤1h), hashed-at-rest tokens, constant-time compare, no token in URL to third party.
...
- **L8. Generated code / scaffolds with default flaws** (M) — review `create-*` starters, especially auth/admin.
...
### Q. AI / LLM Specific (modern web apps)
- **Q1. Prompt injection (direct + indirect)** (H) — separate system/tool channels; never trust retrieved content as instructions.
- **Q2. Sensitive data sent to model providers** (H) — scrub PII/secrets client-side + server-side before prompt.
...
- **Q5. Tool/function-call abuse** (H) — strict allow-list, user-confirm destructive ops, no blanket "send email to address X".
...
- **R1. Deep link / URL scheme hijack** (M) — allow-list hosts; verify signing on Android App Links / iOS Universal Links.
- **R2. WebView JavaScript bridges** (H) — `@JavascriptInterface` allow-list; disable file access; no `setAllowFileAccess(true)`.
...
### S. Compliance & Legal (rapid checklist)
...
Address in this order to maximize risk reduction per hour of effort:
...
- Detailed implementation code (this is an advisory checklist, not a code change plan).
- Threat model for a specific application (would require app context).
